...
async function mapItemToMoment(item) {
  if (item.type === 'collection') {
    if (item.momentsIds) {
      const { moments, createdAt } = await getCollectionItems(item.broadcastId);
...
function convertM3u8ToMp4(inputPath, outputhPath, id = '') {
  return new Promise((resolve, reject) => {
    ...
    converter
      ...
      .on('error', (err) => reject({ message: err.message, id }))
...
async function donwloadAllMoments({ createdBefore, records, channel }) {
  ...
  for (const broadcast of moments.items) {
    ...
    if (type === 'captures') {
      ...
      const firstCapture = moments[0];
      const lastCapture = moments[ moments.length - 1 ];

      const outputDir = makeDirPath(broadcast.createdAt, firstCapture.momentId, lastCapture.momentId);
...
async function downloadBroadcastMoments({ broadCastId }) {
  ...
  const { createdAt, moments } = await getCollectionItems(broadCastId); // TODO:
}
